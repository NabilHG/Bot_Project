...
async def get_monthly_alerts_counts(
    symbols, message, current_api_key_index, current_vpn_server_index
):
    ...
    )  # Si es True, no calcular el backtest
...
async def calculate_maximum_drawdown(
    symbols, message, data_rsi_raw, current_api_key_index, current_vpn_server_index
):
    maximum_drawdown = {}
    print(symbols)
    # if not await is_vpn_active():
            ...
            if ticker in close_prices and ticker in rsis:
                if not math.isnan(rsis[ticker]) and not math.isnan(
                    ...
                    if rsis[ticker] < 25 and cash > 0:
                        buy_notification += 1
                        shares_to_buy = (cash / len(portfolio.keys())) // close_prices[
                            ticker
                        ]
...
async def backtest_handler(message: Message):
    ...
    data, data_rsi_raw = await get_monthly_alerts_counts(
        symbols, message, current_api_key_index, current_vpn_server_index
    )
