...
async def calculate_monthly_avg_alerts(data):
    ...
    average_rsi_above_70 = total_actions_rsi_above_70 / total_months
    average_rsi_below_25 = total_actions_rsi_below_25 / total_months
...
async def backtest_handler(message: Message):
    ...
    text_avg_alerts = (
        ...
        "🔹 Compra: ", Bold(result_avg_alerts['avg_above_70']), "\n"
        "🔹 Venta: ", Bold(result_avg_alerts['avg_below_25'])
    )
    await message.answer(
        **Text(
            f"🧪BackTest\nEmpresas analizadas: {symbols}\n{text_avg_alerts}"
        ).as_kwargs()
    )
    return
